    ...
    <form action="{{route('tour.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->title : __('Add new experience')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url(config('tour.tour_route_prefix') ) }}/<a target="_self" href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("View Class")}}</a>
                    @endif
                </div>
            </div>
            ...
            <div class="lang-content-box">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            <div class="panel">
                                <div class="panel-title"><strong>{{__("Class Featured")}}</strong></div>
                                <div class="panel-body">
                                    <div class="form-group">
