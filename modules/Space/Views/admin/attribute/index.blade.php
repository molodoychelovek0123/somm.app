                                ...
                                @if(count($rows) > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td class="title">
                                                <a target="_self" href="{{route('space.admin.attribute.edit',['id'=>$row->id])}}">{{$row->name}}</a>
                                            </td>
                                            <td>
                                                {{$row->position ?? 0}}
                                            </td>
                                            <td>
                                                <a target="_self" href="{{route('space.admin.attribute.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}
                                                </a>
                                                <a target="_self" href="{{route('space.admin.attribute.term.index',['id'=>$row->id])}}" class="btn btn-sm btn-success"><i class="fa fa"></i> {{__("Manage Terms")}}
                                                </a>
